...
namespace WebSocketServerCore
{
    ...
    public class ChildThread
    {
        ...
        public ChildThread(EventWaitHandle ewh, ref TcpListener webSocketServer, TcpClient client)
        {
            _eStop = ewh;
        ...
        public void Init()
        {
            ...
            while (true)
            {
                ...
                if (_client.Available == 0 && count <= 0)
                    break;
    ...
    public class WebSocketServer
    {
        ...
        public void Start(string serverIp = "127.0.0.1", int serverPort = 3108) 
        {
            ...
            _server.Start();
            Debug.WriteLine("Server has started on , Waiting for a connection...", serverIp, serverPort);

            Task.Run(async () =>
            {
                Debug.WriteLine(" Waiting on clients...OK", Thread.CurrentThread.ManagedThreadId);
            ...
            Debug.WriteLine(" Waiting on clients...Started", Thread.CurrentThread.ManagedThreadId);
        ...
        private void OnAcceptedClient(TcpClient client)
        {
            Debug.WriteLine(" TcpClient Accepted...Start", Thread.CurrentThread.ManagedThreadId);
            ChildThread childThread = new ChildThread(_ewh, ref _server, client);
            Thread thread = new Thread(new ThreadStart(childThread.Init));
            _threads.Add(thread);
            thread.Start();
